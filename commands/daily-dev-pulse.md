...
2. **收集数据** — 运行 `bash ~/.claude/skills/daily-dev-pulse/scripts/daily-dev-pulse.sh --format md $ARGUMENTS`
3. **展示结果** — 将格式化后的早报展示给用户

注意：`$ARGUMENTS` 会将用户输入的参数（如 `--focus security`、`--repos owner/repo`、`--days 30`）直接传递给脚本。
